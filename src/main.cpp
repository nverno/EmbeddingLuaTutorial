...
int main() {
  ...
  printf("---- C++ constructors and destructors -----\n");
  {
    ...
    struct Sprite {
      ...
      Sprite() : x(0), y(0) { numberOfSpritesExisting++; }
      ...
      void Draw() { printf("sprite(%p): x = %d, y = %d\n", this, x, y); }
    };
  ...
  printf("---- user values -----\n");
  {
    ...
    // for (int i = 0; i < 50000; i++)	//comment this line in for benchmarking
    // the mem allocator
    {
      pool.Reset();
      lua_State *L = lua_newstate(ArenaAllocator::l_alloc, &pool);
      // lua_State* L = luaL_newstate();
      // lua_State* L = luaL_newstate(GlobalAllocator::l_alloc, &pool);
  ...
  printf("---- upvalues & lightuserdata -----\n");
  {
    ...
    auto CreateSprite = [](lua_State *L) -> int {
      SpriteManager *sm =
          (SpriteManager *)lua_touserdata(L, lua_upvalueindex(1));
      assert(sm);
