...
inline int PutOnLuaStack(lua_State *) { return 0; }

template <typename T> inline int PutOnLuaStack(lua_State *L, T &toPutOnStack) {
...
inline void CallScriptFunction(lua_State *L, const char *funcName,
                               ARGS &...args) {
  ...
  if (lua_type(L, -1) == LUA_TFUNCTION) {
    ...
    if (lua_pcall(L, numArgs, 0, 0) != 0) {
      printf("unable to call script function '%s', '%s'\n", funcName,
             lua_tostring(L, -1));
      luaL_error(L, "unable to call script function '%s', '%s'", funcName,
                 lua_tostring(L, -1));
    }
  } else {
