...
struct GlobalAllocator {
  void *Allocate(size_t sizeBytes) { return ::operator new(sizeBytes); }
...
struct ArenaAllocator {
  ...
  ArenaAllocator(void *begin, void *end) : m_begin(begin), m_end(end) {
    Reset();
  }
  ...
  void *Allocate(size_t sizeBytes) {
    if (sizeBytes <= MIN_BLOCK_SIZE && m_freeListHead) {
      ...
      m_curr =
          (char *)((uintptr_t)(m_curr + (ALIGNMENT - 1)) & ~(ALIGNMENT - 1));
