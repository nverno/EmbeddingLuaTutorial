...
int ToLua(lua_State *L, rttr::variant &result) {
  ...
  if (result.is_valid() == false) {
    ...
    if (result.is_type<int>()) {
      ...
      luaL_error(L, "unhandled type '%s' being sent to Lua.\n",
                 result.get_type().get_name().to_string().c_str());
    }
  }
...
/*! \brief Invoke #methodToInvoke on #object, passing the arguments to the
 *method from Lua and leave the result on the Lua stack.
 *	- Assumes that the top of the stack downwards is filled with the
 *parameters to the method we are invoking.
 *	- To call a free function pass rttr::instance = {} as #object
 * \return the number of values left on the Lua stack */
int InvokeMethod(lua_State *L, rttr::method &methodToInvoke,
                 rttr::instance &object) {
  rttr::array_range<rttr::parameter_info> nativeParams =
      methodToInvoke.get_parameter_infos();
  ...
  if (numLuaArgs != numNativeArgs) {
    printf("Error calling native function '%s', wrong number of args, expected "
           "%d, got %d\n",
           methodToInvoke.get_name().to_string().c_str(), numNativeArgs,
           numLuaArgs);
  ...
  for (int i = 0; i < numLuaArgs; i++, nativeParamsIt++) {
    ...
    default:
      luaL_error(
          L, "Don't know this lua type '%s', parameter %d when calling '%s'",
          lua_typename(L, luaType), i,
...
int InvokeFuncOnUserDatum(lua_State *L) {
  ...
  if (lua_isuserdata(L, 1) == false) {
    luaL_error(L,
               "Expected a userdatum on the lua stack when invoking native "
               "method '%s'",
...
int IndexUserDatum(lua_State *L) {
  ...
  if (lua_isuserdata(L, 1) == false) {
    luaL_error(
        L,
        "Expected a userdatum on the lua stack when indexing native type '%s'",
        typeName);
  }

  if (lua_isstring(L, 2) == false) {
    luaL_error(L,
               "Expected a name of a native property or method when indexing "
               "native type '%s'",
               typeName);
  }
  ...
  if (p.is_valid()) {
    ...
    case LUA_TNUMBER:
      if (p.get_type() == rttr::type::get<int>()) {
        ...
        luaL_error(L,
                   "Cannot set the value '%s' on this type '%s', we didn't "
                   "recognise the native type '%s'",
                   fieldName, typeName,
                   p.get_type().get_name().to_string().c_str());
      }
      break;
    default:
      luaL_error(L,
                 "Cannot set the value '%s' on this type '%s', we didnt "
                 "recognise the lua type '%s'",
...
int ExecuteScript(lua_State *L) { return lua_pcall(L, 0, LUA_MULTRET, 0); }

void CloseScript(lua_State *L) { lua_close(L); }
